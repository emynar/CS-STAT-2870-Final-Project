...
for (i in 1:(nrow(fit_stats_norm)-1)){
  loop_knn <-
    knn.reg(
      train = num_species_norm |> dplyr::select(acres),
      y = num_species_norm$num_species,
      k = i)
...
for (i in 1:(nrow(fit_stats_stan)-1)){
  loop_knn_1 <-
    knn.reg(
      train = num_species_stan |> dplyr::select(acres),
      y = num_species_stan$num_species,
      k = i)
  
  fit_stats_stan[i, "R2"] <- loop_knn_1$R2Pred
  fit_stats_stan[i, "MAE"] <- (species_and_acres$num_species - loop_knn_1$pred) |> abs() |> mean()
