...
```
As seen, the vast majority of species observed fall into the **bird** or **insect** categories. This is expected, but for different reasons for each.

1) Insects: have by far the highest species diversity of any group.

2) Birds: Because we removed seasonality, we are looking at species spotted in the park at any time of year, including species that are strictly migratory. Due to their migratory nature, the average bird species is often has a wider range of observation than that of an equivalent reptile, amphibian or mammal. Additionally, due to a variety of factors, birds are often have more comprehensive records than any other group. The MAPS program, created by the Institute for Bird Populations has gone a long way to creating distribution patterns across the United States, and the role of citizen science, on programs like eBird or iNaturalist create lists of observed species without the need for a monitoring program run by the park. 
```{r species by park}
...
```
This graph reveals an issue with our data. The greatest predictor of species diversity is that of **insect diversity**. This makes sense - insects have by far the greatest species diversity on large. However, several National Parks reported *no insects* in their data. There is the possibility that certain sites observed no insects - 
