...
```
However, raw number of species does not tell a manager the whole picture. Certain spcies require more help than others...
...
```
```{r endangered species proportion graph} 

# clearly a work in progress, having some issues with the aes mapping. Will work on it later
ggplot(data = proportion,
       mapping = aes(x = proportion_endangered)) +
...
## work in progress
left_join(species_per_park, endangered_species_per_park, by = "park_name") |>
  mutate(proportion_endangered = (num_species.y / num_species.x) * 100) |>
  select(name = park_name, proportion_endangered) |>
  arrange(desc(proportion_endangered)) ->
  proportion 
  tibble(proportion)
